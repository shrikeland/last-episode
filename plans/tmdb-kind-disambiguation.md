...
## После мержа main (конфликты в PR #19)
В `main` пришли фичи с новыми выборками по одному `tmdb_id`. Все переведены на пару (kind, tmdb_id):
- `getFriendsWithTitle` (блок «У друзей»): иначе на карточке сериала N показывались друзья с фильмом N.
- `getMediaItemIdByTmdbId` (ссылка «Моя карточка»): `.maybeSingle()` по одному `tmdb_id` падал бы на двух строках (фильм N и сериал N), и ссылка пропадала.
- `TitleRecommendations.libraryItemIds`: ключ `mediaTitleKey` вместо `tmdb_id`. Для этого добавлен экшен `getLibraryItemIds` (ключ → id записи); `getLibraryTitleKeys` теперь его обёртка.
- `lib/compare.ts` уже сравнивал по kind; его `libraryKey` переведён на общий `mediaTitleKey`, формат ключа тот же.
